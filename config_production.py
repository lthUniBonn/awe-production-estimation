...
sample_ids = [  217,   295,   492,   895,   965,  1087,  1596,  1840,  2166, 2697,  2705,  3671,  4163,  4289,  4315,  4877,  5097,  5132, 5213,  5321,  5358,  5543,  6139,  6179,  6978,  7251,  7380, 7444,  7468,  7795,  7806,  8006,  8124,  8176,  8294,  8346, 8427,
...
# Running on a job:
import os
jobnum = 0 #int(os.environ['LOCNUM'])
step=1
location_number = -1 #jobnum*step # -1 #select loctions up to location number, -1: all locs #up to +400

print('Set Jobnumber/Locnumber: ', jobnum, 'in steps of ', step)


def get_loc_brute_force_name_and_locs(location_number, data_info, mult_loc = True):
    #location_number=10 # -1 #select loctions up to location number, -1: all locs in mult loc case
...
brute_force_testing_file_name = result_dir + brute_force_files.format(sample_selection, data_info, 'pickle')
